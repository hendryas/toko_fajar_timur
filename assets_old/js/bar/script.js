	...
	if (input_stok == undefined || input_stok == "") {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: "bar/insert_data_stok_bar/bar.php",
					method: "POST",
					data: formData,
					cache: false,
					contentType: false,
...
$("button[name='btnSaveSisaPack']").click(function (e) {
	...
	if (input_sisa_ecer == undefined || input_sisa_ecer == "") {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: "bar/insert_data_sisa_stok_bar/bar.php",
					method: "POST",
					data: formData,
					cache: false,
					contentType: false,
...
$("button[name='btnTambahStokBarLangsung']").click(function (e) {
	let id_rumah = $(this).closest("tr").find("#id_rumah").val();
	let id_bar = $(this).closest("tr").find("#id_bar").val();
	let stok_awal_qty = $(this).closest("tr").find("#stok_awal_qty").val();
	let stok_awal_pack = $(this).closest("tr").find("#stok_awal_pack").val();
	...
	let tanggal_input_stok = $(this)
		...
		.find("#tanggal_input_stok")
		.val();
	let input_stok_qty_bar = $(this)
		.closest("tr")
		.find("#input_stok_qty_bar")
		.val();
	let tambah_qty = $(this).closest("tr").find("#tambah_qty").val();
	...
	console.log(stok_awal_harga);
	console.log(tanggal_input_stok);
	console.log(input_stok_qty_bar);
	console.log(tambah_qty);
	...
	formData.append("stok_awal_harga", stok_awal_harga);
	formData.append("tanggal_input_stok", tanggal_input_stok);
	formData.append("input_stok_qty_bar", input_stok_qty_bar);
	formData.append("tambah_qty", tambah_qty);

	if (input_stok_qty_bar == undefined || input_stok_qty_bar == "") {
		Swal.fire({
			title: "Inputan Kosong!",
			text: "Kolom input stok bar langsung tidak boleh kosong!",
			icon: "question",
			confirmButtonColor: "#5664d2",
		});
	} else {
		Swal.fire({
			title: "Apakah Anda Yakin?",
			text: "Ingin Menambahkan Stok Bar Langsung!",
			icon: "question",
			showCancelButton: true,
			confirmButtonColor: "#19A87E",
			cancelButtonColor: "#ff3d60",
			...
			if (result.isConfirmed) {
				$.ajax({
					url: "bar/insert_data_stok_bar_langsung/bar.php",
					method: "POST",
					data: formData,
					cache: false,
					contentType: false,
...
$("button[name='btnSaveCatatanTambah']").click(function (e) {
	...
	if (catatan == undefined || catatan == "") {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: "bar/insert_catatan_tambah_stok_bar/bar.php",
					method: "POST",
					data: formData,
					cache: false,
					contentType: false,
...
$("button[name='btnSaveCatatanSisa']").click(function (e) {
	...
	if (catatan == undefined || catatan == "") {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: "bar/insert_catatan_sisa_stok_bar/bar.php",
					method: "POST",
					data: formData,
					cache: false,
					contentType: false,
